import{c as w,r as m,s as l,j as e,k as g,T as F,A as C,m as A}from"./index-BKXChAKI.js";import{L as x}from"./loader-circle-DztntJTj.js";import{D as M}from"./download-BN8sdeof.js";import{Z as X}from"./zap-BxxwZcGo.js";import{A as k}from"./activity-BeI_r18i.js";import{T as I}from"./terminal-0LgiyEkL.js";import{C as v}from"./circle-check-big-DqDI4SE0.js";/**
...
`;function z(){const[_,h]=m.useState(!1),[b,u]=m.useState(!1),[U,o]=m.useState([{id:"initial",time:new Date().toLocaleTimeString(),msg:"System initialized. Realtime stream active.",type:"system"}]),[d,S]=m.useState({latency:0,activeRequests:0,blockedThreats:0,healthScore:100,history:[40,50,60,40,70,80,50,60],lastScan:new Date().toLocaleTimeString()}),N=async()=>{const t=performance.now();try{const[{count:a},{count:n},{count:s},{count:i}]=await Promise.all([l.from("payment_submissions").select("*",{count:"exact",head:!0}).eq("status","pending"),l.from("bill_submissions").select("*",{count:"exact",head:!0}).eq("status","pending"),l.from("kyc_submissions").select("*",{count:"exact",head:!0}).eq("status","pending"),l.from("users_profiles").select("*",{count:"exact",head:!0}).eq("status","Blocked")]),r=performance.now(),c=Math.round(r-t),T=(a||0)+(n||0)+(s||0),p=i||0;S(L=>({latency:c,activeRequests:T,blockedThreats:p,healthScore:Math.max(70,100-T*2-(c>500?10:0)),history:[...L.history.slice(1),Math.min(100,30+T*10+c/10)],lastScan:new Date().toLocaleTimeString()}))}catch(a){console.error("Metrics fetch error:",a)}};m.useEffect(()=>{N();const t=setInterval(N,5e3);return()=>clearInterval(t)},[]),m.useEffect(()=>{const t=(s,i="db")=>{o(r=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:s,type:i},...r.slice(0,49)])},a=()=>{const s=["User Login: Session Started","DB/Query: SELECT * FROM admin_profiles","System/Kernel: Optimization Complete","API/Sync: WebSocket connected"];t(s[Math.floor(Math.random()*s.length)],"system")};window.simulateDevLog=a;const n=[l.channel("logs-users").on("postgres_changes",{event:"INSERT",schema:"public",table:"users_profiles"},s=>{t(`User Registered: ${s.new.name||s.new.id}`)}),l.channel("logs-qr").on("postgres_changes",{event:"INSERT",schema:"public",table:"payment_submissions"},s=>{t(`New QR Payment: ₹${s.new.amount} from ${s.new.user_id}`)}),l.channel("logs-bill").on("postgres_changes",{event:"INSERT",schema:"public",table:"bill_submissions"},s=>{t(`New Bill Payment: ₹${s.new.amount}`)}),l.channel("logs-kyc").on("postgres_changes",{event:"UPDATE",schema:"public",table:"kyc_submissions"},s=>{t(`KYC ${s.new.status}: ${s.new.user_id}`)}),l.channel("logs-system").on("postgres_changes",{event:"UPDATE",schema:"public",table:"system_status"},s=>{t(`SYSTEM STATUS UPDATED: ${s.new.message}`,"system")})];return n.forEach(s=>s.subscribe()),()=>{n.forEach(s=>l.removeChannel(s))}},[]);const R=async()=>{h(!0);try{const t=["users_profiles","qr_history","rejection_categories","rejection_reasons","admin_profiles","admin_withdrawals","app_policies","bank_details","bill_submissions","complaint_messages","complaints","distributor_withdrawals","headlines","kyc_submissions","notifications","onesignal_settings","payment_submissions","payout_settings","payout_submissions","qr_settings","service_charge_slabs","system_status","whatsapp_api_settings"];let a=`-- MASTER DATABASE EXPORT (TOTAL SYSTEM CLONE)
...
`;const L=c.map(O=>`(${p.map(D=>{const E=O[D];return E===null?"NULL":typeof E=="number"||typeof E=="boolean"?E:typeof E=="object"?`'${JSON.stringify(E).replace(/'/g,"''")}'`:`'${String(E).replace(/'/g,"''")}'`}).join(", ")})`);a+=L.join(`,
`)+`;
`}else a+=`-- No data found in ${r}
`}const n=new Blob([a],{type:"text/sql"}),s=window.URL.createObjectURL(n),i=document.createElement("a");i.href=s,i.download=`UsePay_Master_Full_Backup_${new Date().toISOString().split("T")[0]}.sql`,document.body.appendChild(i),i.click(),window.URL.revokeObjectURL(s),document.body.removeChild(i),o(r=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:"Master SQL Full Database Export successful!",type:"system"},...r])}catch(t){console.error("Export error:",t)}finally{h(!1)}},y=async()=>{u(!0),o(t=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:"Preparing full system backup (Database + Storage)...",type:"system"},...t]);try{const t=await fetch("/api/full-backup");if(!t.ok)throw new Error("Backup failed on server");const a=await t.blob(),n=window.URL.createObjectURL(a),s=document.createElement("a");s.href=n,s.download=`UsePay_Full_Backup_${new Date().toISOString().replace(/[:.]/g,"-")}.zip`,document.body.appendChild(s),s.click(),window.URL.revokeObjectURL(n),document.body.removeChild(s),o(i=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:"Full System Backup ZIP downloaded successfully!",type:"system"},...i])}catch(t){console.error("Full Backup error:",t),o(a=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:`Backup Failed: ${t.message}`,type:"system"},...a])}finally{u(!1)}},f=async()=>{u(!0),o(t=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:"Starting Quick SQL-only backup...",type:"system"},...t]);try{const t=await fetch("/api/full-backup?mode=quick");if(!t.ok)throw new Error("Backup failed");const a=await t.blob(),n=window.URL.createObjectURL(a),s=document.createElement("a");s.href=n,s.download=`UsePay_Quick_SQL_${new Date().toISOString().split("T")[0]}.zip`,document.body.appendChild(s),s.click(),window.URL.revokeObjectURL(n),document.body.removeChild(s),o(i=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:"Quick SQL Backup successful!",type:"system"},...i])}catch(t){o(a=>[{id:Math.random().toString(36).substr(2,9),time:new Date().toLocaleTimeString(),msg:`Quick Backup Failed: ${t.message}`,type:"system"},...a])}finally{u(!1)}};return e.jsxs("div",{className:"space-y-6",children:[e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsxs("div",{children:[e.jsx("h2",{className:"text-2xl font-bold text-slate-900",children:"System Logs"}),e.jsx("p",{className:"text-slate-500 mt-1",children:"Real-time developer analytics and system health."})]}),e.jsxs("div",{className:"flex items-center gap-3",children:[e.jsxs("button",{onClick:R,disabled:_,className:"flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-lg shadow-emerald-200 transition-all active:scale-95 font-bold text-xs uppercase tracking-widest disabled:opacity-50",children:[_?e.jsx(x,{size:14,className:"animate-spin"}):e.jsx(M,{size:14}),"Master SQL Export"]}),e.jsxs("div",{className:"flex items-center bg-slate-100 rounded-xl p-1 gap-1",children:[e.jsxs("button",{onClick:y,disabled:b,className:"flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-black text-white rounded-lg shadow-sm transition-all active:scale-95 font-bold text-[10px] uppercase tracking-widest disabled:opacity-50",children:[b?e.jsx(x,{size:12,className:"animate-spin"}):e.jsx(g,{size:12}),"Full (ZIP)"]}),e.jsx("button",{onClick:f,disabled:b,className:"flex items-center gap-2 px-4 py-2 bg-white hover:bg-slate-50 text-slate-900 rounded-lg shadow-sm border border-slate-200 transition-all active:scale-95 font-bold text-[10px] uppercase tracking-widest disabled:opacity-50",children:"Quick (SQL)"})]}),e.jsxs("button",{onClick:()=>window.simulateDevLog(),className:"flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-95 font-bold text-xs uppercase tracking-widest",children:[e.jsx(X,{size:14}),"Test Activity"]}),e.jsxs("button",{onClick:()=>o([{id:"clear",time:new Date().toLocaleTimeString(),msg:"Console cleared by Developer.",type:"system"}]),className:"flex items-center gap-2 px-4 py-2 bg-white hover:bg-slate-50 text-slate-600 rounded-xl border border-slate-200 shadow-sm transition-all active:scale-95 font-bold text-xs uppercase tracking-widest",children:[e.jsx(F,{className:"w-3.5 h-3.5"}),"Clear"]}),e.jsxs("div",{className:"flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl border border-emerald-100 shadow-sm font-bold text-xs uppercase tracking-widest animate-pulse",children:[e.jsx(k,{size:14}),"Live Kernels: Active"]})]})]}),e.jsxs("div",{className:"grid grid-cols-1 md:grid-cols-3 gap-6",children:[e.jsxs("div",{className:"bg-white p-6 rounded-3xl border border-slate-100 shadow-sm",children:[e.jsx("div",{className:"w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 mb-4",children:e.jsx(I,{size:24})}),e.jsx("h3",{className:"font-bold text-slate-900 text-lg",children:"Event Stream"}),e.jsx("p",{className:"text-slate-500 text-sm mt-1",children:"Monitor all database interactions."}),e.jsx("div",{className:"mt-4 p-4 bg-slate-900 rounded-2xl font-mono text-[10px] space-y-2 h-[200px] overflow-y-auto custom-scrollbar border border-slate-800",children:e.jsx(C,{initial:!1,children:U.map(t=>e.jsxs(A.div,{initial:{opacity:0,x:-10},animate:{opacity:1,x:0},className:"flex items-start gap-2",children:[e.jsxs("span",{className:"text-slate-600 shrink-0",children:["[",t.time,"]"]}),e.jsx("span",{className:t.type==="system"?"text-rose-400 font-bold":"text-emerald-400",children:t.msg})]},t.id))})})]}),e.jsxs("div",{className:"bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden",children:[e.jsx("div",{className:"w-12 h-12 bg-rose-50 rounded-2xl flex items-center justify-center text-rose-600 mb-4",children:e.jsx(g,{size:24})}),e.jsx("h3",{className:"font-bold text-slate-900 text-lg",children:"Security Integrity"}),e.jsx("p",{className:"text-slate-500 text-sm mt-1",children:"Live firewall status."}),e.jsxs("div",{className:"mt-6 space-y-4",children:[e.jsxs("div",{className:"grid grid-cols-2 gap-4 pb-4 border-b border-slate-50",children:[e.jsxs("div",{children:[e.jsx("p",{className:"text-[10px] font-black text-slate-400 uppercase tracking-widest",children:"Blocked Threats"}),e.jsx("p",{className:"text-lg font-black text-rose-600 tracking-tight",children:d.blockedThreats})]}),e.jsxs("div",{children:[e.jsx("p",{className:"text-[10px] font-black text-slate-400 uppercase tracking-widest",children:"Last Scan"}),e.jsx("p",{className:"text-[11px] font-bold text-slate-900 mt-1",children:d.lastScan})]})]}),e.jsxs("div",{className:"flex items-center justify-between text-xs font-bold pt-2",children:[e.jsx("span",{className:"text-slate-400 uppercase tracking-widest",children:"Firewall Integrity"}),e.jsxs("span",{className:"text-emerald-500",children:[d.healthScore,"%"]})]}),e.jsx("div",{className:"w-full bg-slate-100 h-2 rounded-full overflow-hidden",children:e.jsx(A.div,{initial:{width:0},animate:{width:`${d.healthScore}%`},className:"h-full bg-emerald-500"})}),e.jsx("div",{className:"pt-2",children:e.jsxs("p",{className:"text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-2",children:[e.jsx(v,{size:10,className:"text-emerald-500"})," Active Protection Enabled"]})})]})]}),e.jsxs("div",{className:"bg-white p-6 rounded-3xl border border-slate-100 shadow-sm",children:[e.jsx("div",{className:"w-12 h-12 bg-amber-50 rounded-2xl flex items-center justify-center text-amber-600 mb-4",children:e.jsx(B,{size:24})}),e.jsx("h3",{className:"font-bold text-slate-900 text-lg",children:"System Load"}),e.jsx("p",{className:"text-slate-500 text-sm mt-1",children:"Core performance metrics."}),e.jsxs("div",{className:"mt-4 grid grid-cols-2 gap-4 border-b border-slate-50 pb-4 mb-4",children:[e.jsxs("div",{children:[e.jsx("p",{className:"text-[10px] font-black text-slate-400 uppercase tracking-widest",children:"Latency"}),e.jsxs("p",{className:"text-lg font-black text-slate-900 tracking-tight",children:[d.latency,"ms"]})]}),e.jsxs("div",{children:[e.jsx("p",{className:"text-[10px] font-black text-slate-400 uppercase tracking-widest",children:"Pending"}),e.jsxs("p",{className:"text-lg font-black text-slate-900 tracking-tight",children:[d.activeRequests," req"]})]})]}),e.jsx("div",{className:"flex items-end gap-1 h-10",children:d.history.map((t,a)=>e.jsx(A.div,{initial:{height:0},animate:{height:`${t}%`},className:`flex-1 rounded-t-sm ${t>80?"bg-rose-400":t>50?"bg-amber-400":"bg-indigo-400"}`},a))})]})]}),e.jsxs("div",{className:"bg-slate-900 rounded-[32px] p-10 text-center border border-slate-800 shadow-2xl relative overflow-hidden",children:[e.jsx("div",{className:"absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20"}),e.jsxs("div",{className:"relative z-10",children:[e.jsx(I,{className:"text-indigo-500 mx-auto mb-6",size:48}),e.jsx("h2",{className:"text-3xl font-black text-white tracking-tight mb-4",children:"Developer Sandbox"}),e.jsxs("p",{className:"text-slate-400 max-w-lg mx-auto leading-relaxed",children:["This panel is currently in ",e.jsx("b",{children:"Stealth Mode"}),". Only authorized developers with the encrypted ID can view this screen. System telemetry is being recorded."]})]})]})]})}export{z as default};
