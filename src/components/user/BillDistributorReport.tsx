...
interface BillDistributorReportProps {
  userId: string;
}

export default function BillDistributorReport({ userId }: BillDistributorReportProps) {
  ...
  const [cardOrMobileSearch, setCardOrMobileSearch] = useState('');
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          users_profiles!bill_submissions_user_id_fkey(id, name, firm_name, distributor_id)
        `)
        .in('user_id', userIds);
  ...
  const filteredRequests = requests.filter(req => {
    ...
    const matchesCardOrMobile = !cardOrMobileSearch || 
      req.customer_mobile.includes(cardOrMobileSearch) ||
      req.card_number.includes(cardOrMobileSearch) ||
      req.card_owner_name.toLowerCase().includes(cardOrMobileSearch.toLowerCase());
    
    const matchesAmount = !amountSearch || 
      req.amount.toString().includes(amountSearch);

    return matchesDist && matchesUser && matchesCardOrMobile && matchesAmount;
  });
  ...
  const exportToExcel = () => {
    const exportData = filteredRequests.map(req => ({
      ...
      'Card Bank': req.card_bank,
      'Card Number': req.card_number,
      'Owner Name': req.card_owner_name,
      'Amount (Rs)': Number(req.amount || 0),
      'Charges (Rs)': Number(req.charges || 0),
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Bill Distributor Report");
    XLSX.writeFile(wb, `Bill_Distributor_Report_${new Date().toLocaleDateString()}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Bill Distributor Report</h2>
          <p className="text-slate-500 mt-1">Bill payment request reports of users under your distributor network.</p>
        </div>
        <button 
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
          ...
          {/* Card / Mobile Search */}
          <div className="space-y-1.5 lg:col-span-1">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Mobile / Card / Owner</label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
              <input
                type="text"
                value={cardOrMobileSearch}
                onChange={(e) => setCardOrMobileSearch(e.target.value)}
                placeholder="Search mobile, card..."
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Card Details</th>
