...
interface UserMPINProps {
  userId: string;
}

export default function UserMPIN({ userId }: UserMPINProps) {
  const toast = useToast();
  
  // MPIN settings state
  const [hasMpin, setHasMpin] = useState(false);
  const [dbMpin, setDbMpin] = useState<string | null>(null);
  ...
  const fetchMpinStatus = async () => {
    ...
    try {
      const { data, error: profileErr } = await supabase
        .from('users_profiles')
        .select('mpin, password, name, email')
        .eq('id', userId)
        .single();
      ...
      if (!profileErr && data) {
        setUserProfile(data);
        setDbMpin(data.mpin);
        setHasMpin(!!data.mpin && data.mpin.trim().length === 6);
      }
    } catch (err) {
  ...
  const handleCreateOrChangeMpin = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    setSuccessMsg(null);
    ...
    if (newMpin.length !== 6 || !/^\d{6}$/.test(newMpin)) {
      setError('New MPIN must be exactly 6 digits.');
      return;
    }
    ...
    try {
      // 1. Verify password & current MPIN (if exists)
      if (userProfile.password !== password) {
        throw new Error('Account password is incorrect.');
      }

      if (hasMpin && !isForgotMode) {
        if (!currentMpin || currentMpin.length !== 6) {
          throw new Error('Please enter your current 6-digit MPIN.');
        }
        if (dbMpin !== currentMpin) {
      ...
      setSuccessMsg(isForgotMode ? 'MPIN reset successfully!' : (hasMpin ? 'MPIN changed successfully!' : 'MPIN created successfully!'));
      toast.success(isForgotMode ? 'MPIN Reset' : (hasMpin ? 'MPIN Changed' : 'MPIN Created'));
      ...
      console.error('MPIN action error:', err);
  ...
  const handleForgotMpin = async () => {
    if (!hasMpin || !dbMpin) {
      toast.error('You have not created an MPIN yet.');
      return;
    }
    ...
    try {
        ...
        body: JSON.stringify({
          ...
          html: `
            <div style="font-family: sans-serif; padding: 20px; color: #333; max-width: 600px; margin: auto; border: 1px solid #e2e8f0; border-radius: 16px;">
              <h2 style="color: #059669; margin-bottom: 24px;">Login MPIN Recovery</h2>
              ...
              <div style="background: #f0fdf4; padding: 24px; border-radius: 12px; margin: 24px 0; border: 1px solid #dcfce7; text-align: center;">
                <p style="margin: 8px 0; font-size: 14px; text-transform: uppercase; letter-spacing: 0.1em; color: #047857; font-weight: bold;">Your 6-Digit MPIN</p>
                <p style="margin: 0; font-size: 36px; font-weight: 900; letter-spacing: 0.3em; color: #065f46; font-family: monospace;">${dbMpin}</p>
              </div>
      ...
      setSuccessMsg('Your MPIN has been sent to your registered email address.');
      toast.success('MPIN Recovery Mail Sent');
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Loading MPIN settings...</p>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto py-8 px-4">
      <div className="mb-10 text-center">
        ...
        <h2 className="text-3xl font-black text-slate-900 tracking-tight">
          {isForgotMode ? 'Reset MPIN' : (hasMpin ? 'Login Security (MPIN)' : 'Create Login MPIN')}
        </h2>
        <p className="text-slate-500 mt-2 font-medium">
          {isForgotMode ? 'Reset your Login MPIN using your account password.' : (hasMpin ? 'Change or recover your 6-digit Login MPIN.' : 'Set up a secure 6-digit Mobile PIN to protect your dashboard.')}
        </p>
      </div>
      ...
      >
        <form onSubmit={handleCreateOrChangeMpin} className="space-y-8">
          <div className="space-y-6">
            
            {/* If changing MPIN, ask for Current MPIN */}
            {hasMpin && !isForgotMode && (
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={currentMpin}
                    onChange={(e) => handleInputChange(e.target.value, setCurrentMpin)}
                    placeholder="••••••"
            ...
            {/* New MPIN & Confirm MPIN */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="group">
                <label className="block text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1 transition-colors group-focus-within:text-emerald-500">
                  {hasMpin ? 'New 6-Digit MPIN' : 'Create 6-Digit MPIN'}
                </label>
                <div className="relative">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            {hasMpin && (
              ...
              >
                {isForgotMode ? 'Back to Change' : 'Forgot MPIN?'}
              </button>
            )}
            ...
            >
              {loading ? (
                ...
                <>
                  <ShieldCheck size={20} />
                  {isForgotMode ? 'Reset MPIN' : (hasMpin ? 'Change MPIN' : 'Create MPIN')}
                </>
              )}
            </button>
          </div>
          ...
          <p className="text-[10px] text-center text-slate-400 uppercase font-black tracking-widest">
            Always keep your MPIN confidential • Security checks logged
          </p>
