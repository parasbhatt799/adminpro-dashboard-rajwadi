  ...
  const handleConfirmRefund = async () => {
    ...
    try {
      const res = await fetch('/api/payout/admin-refund', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          payoutId: refundModalTx.id,
          txn_id: refundModalTx.bank_ref || refundModalTx.txn_id,
          reason: refundReason
        })
      });
      const data = await res.json();
      if (data.success) {
        setMessage({
          type: 'success',
          text: `✅ ${data.message || 'Refund processed successfully.'} (Amount + Charge credited to user wallet)`
        });
  ...
  return (
    <div className="space-y-6">
                      ...
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        {!['rejected', 'failed', 'refunded'].includes((tx.status || '').toLowerCase()) ? (
                          <button
                            ...
                            className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded-lg border border-rose-200 transition-colors inline-flex items-center gap-1 cursor-pointer shadow-2xs"
                            title="Refund Amount + Charge to User Wallet"
                          >
                          ...
                          <span
                            className="px-2 py-1 bg-slate-100 text-slate-400 text-[10px] font-bold rounded-lg border border-slate-200 select-none inline-flex items-center gap-1"
                            title="This transaction has already been refunded / rejected"
                          >
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
              <div className={`p-3 rounded-2xl ${
                ...
                (selectedTx.status === 'rejected' || selectedTx.status === 'failed' || selectedTx.status === 'refunded') ? 'bg-rose-50 text-rose-600 border border-rose-100' :
                'bg-amber-50 text-amber-600 border border-amber-100'
              }`}>
                {selectedTx.status === 'approved' ? <CheckCircle2 size={24} /> :
                 (selectedTx.status === 'rejected' || selectedTx.status === 'failed' || selectedTx.status === 'refunded') ? <XCircle size={24} /> :
                 <Clock size={24} />}
              </div>
              <div>
            ...
            {/* Failure Reason Alert Box if Rejected or Refunded */}
            {(selectedTx.status === 'rejected' || selectedTx.status === 'failed' || selectedTx.status === 'refunded') && (
      ...
      {refundModalTx && (
        <div 
          className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm"
          onClick={() => !refunding && setRefundModalTx(null)}
        >
          <motion.div 
            initial={{ opacity: 0, scale: 0.95, y: 15 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
              ...
              <div>
                <h3 className="font-black text-slate-900 text-lg">Refund Payout to User</h3>
                <p className="text-xs text-slate-500 font-medium font-mono">
                  Ref: {refundModalTx.bank_ref || refundModalTx.txn_id || refundModalTx.id}
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2.5 text-xs">
              <div className="flex justify-between items-center text-slate-600">
                <span className="font-medium">User / Firm:</span>
                <span className="font-bold text-slate-900">
                  {refundModalTx.users_profiles?.firm_name || refundModalTx.users_profiles?.name || 'N/A'}
                </span>
              </div>
              <div className="flex justify-between items-center text-slate-600">
                <span className="font-medium">Mobile Number:</span>
                <span className="font-bold text-slate-900 font-mono">
                  {refundModalTx.users_profiles?.mobile_number || 'N/A'}
                </span>
              </div>
              <div className="flex justify-between items-center text-slate-600">
                <span className="font-medium">Beneficiary Details:</span>
                <span className="font-bold text-slate-900 font-mono text-right">
                  {refundModalTx.account_holder_name || refundModalTx.beneficiary_name || refundModalTx.holder_name || 'N/A'}
                  <span className="block text-[11px] text-slate-500 font-normal">
                    {refundModalTx.account_number || 'N/A'} {refundModalTx.ifsc_code ? `(${refundModalTx.ifsc_code})` : ''}
                  </span>
                </span>
              </div>
              ...
              <div className="flex justify-between items-center text-slate-600">
                ...
                <span className="font-bold text-slate-900">
                  ₹{Number(refundModalTx.amount || 0).toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between items-center text-slate-600">
                <span className="font-medium">Service Charge:</span>
                <span className="font-bold text-rose-600">
                  + ₹{Number(refundModalTx.charge_amount || 0).toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between items-center border-t border-slate-200 pt-2 text-sm font-bold">
                ...
                <span className="text-emerald-700 font-black text-base">
                  ₹{(Number(refundModalTx.amount || 0) + Number(refundModalTx.charge_amount || 0)).toFixed(2)}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 pt-2">
              ...
              >
                {refunding ? (
                  ...
                  <>
                    <RotateCcw className="w-3.5 h-3.5" />
                    Confirm & Refund ₹{(Number(refundModalTx.amount || 0) + Number(refundModalTx.charge_amount || 0)).toFixed(2)}
                  </>
                )}
              </button>
            </div>
