...
export default async function handler(req: any, res: any) {
  ...
  console.log(`[QR Email Vercel Route] Triggered for ID: ${requestId}, status: ${status}`);
  ...
  try {
    ...
    if (!user.email || !user.email.includes("@")) {
      console.warn(`[QR Email Vercel Route] Skipping email. User ${user.id} does not have a valid email: ${user.email}`);
    ...
    console.error("[QR Email Vercel Route] Failed:", err);
